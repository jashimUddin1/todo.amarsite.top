<?php
// todo_list.php — Range + Live Timer + Single-running + Off Day (Worked/OT) + Column Management
...
/* Helpers */
function flash($text, $type='dark') { $_SESSION['flash'] = ['text'=>$text, 'type'=>$type]; }

function fmt_hr_min($durStr) {
  if (!$durStr) return '';
  if (preg_match('/\d+/', (string)$durStr, $m)) {
    $mins = (int)$m[0];
...
function fmt_range($from, $to){
  if (!$from && !$to) return '—';
  if ($from && !$to)  return date('d', strtotime($from));
  if (!$from && $to)  return date('d', strtotime($to));

  $f = strtotime($from);
  $t = strtotime($to);

  // If same month & same year → show 01–07
  if (date('mY', $f) === date('mY', $t)) {
    return date('d', $f) . '–' . date('d', $t);
  }

  // Different month/year → show full range
...
/* Did we work on this off-day range? (manual mark or daily_reports activity) */
function off_worked_effective($row, mysqli $con){
  if (!is_off_day_title($row['title'])) return false;
  if ((int)$row['off_worked'] === 1)    return true;
...
function status_badge($row, mysqli $con){
  ...
  if (!empty($row['running_start']))   return '<span class="badge text-bg-primary">In&nbsp;Progress</span>';
  return ((int)$row['status'] === 1)
    ? '<span class="badge text-bg-success">Completed</span>'
    : '<span class="badge text-bg-info">Pending</span>';
}
function pr_badge($p){
  if ($p>=3) return '<span class="badge text-bg-danger">High</span>';
  if ($p==2) return '<span class="badge text-bg-warning">Medium</span>';
...
if ($res && $res->num_rows) $runningAny = $res->fetch_assoc();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($action === 'save_settings') {
    ...
    header("Location: todo_list.php".keep_month_qs()); exit;
  }
  ...
  if ($action === 'create') {
    ...
    if ($title === '' || !$date_from) { flash('From Date & Description required','warning'); header("Location: todo_list.php"); exit; }
    if ($date_from && $date_to && strtotime($date_to) < strtotime($date_from)) { $tmp=$date_from; $date_from=$date_to; $date_to=$tmp; }
    ...
    $y = date('Y', strtotime($date_from)); $m = date('n', strtotime($date_from));
    header("Location: todo_list.php?year=$y&month=$m"); exit;
  }
  ...
  if ($action === 'update') {
    ...
    if ($id<=0 || $title==='' || !$date_from) { flash('Missing required fields','warning'); header("Location: todo_list.php"); exit; }
    if ($date_from && $date_to && strtotime($date_to) < strtotime($date_from)) { $tmp=$date_from; $date_from=$date_to; $date_to=$tmp; }
  ...
  if ($action === 'start_timer') {
    ...
    if ($id>0) {
      // fetch this task (title, off_worked)
      $st = $con->prepare("SELECT title, off_worked FROM todos WHERE id=?");
      $st->bind_param("i",$id); $st->execute();
      $row = $st->get_result()->fetch_assoc();

      // Off Day cannot start unless worked is effective
      if ($row && is_off_day_title($row['title'])) {
        // check effective worked (manual or daily_reports)
        $tinfo = $con->prepare("SELECT * FROM todos WHERE id=?");
        $tinfo->bind_param("i",$id); $tinfo->execute();
      ...
      if ($r2 && $r2->num_rows) {
        ...
        if ((int)$other['id'] !== $id) {
          ...
          // Auto-stop & log the running one
          $startDT = strtotime($other['running_start']); $endDT = time();
          if ($endDT > $startDT) {
  ...
  if ($action === 'end_timer') {
    ...
    if ($id>0) {
      ...
      if ($task && $task['running_start']) {
        $startDT = strtotime($task['running_start']); $endDT = time();
        if ($endDT > $startDT) {
          $mins        = (int) round(($endDT - $startDT) / 60);
          $report_date = date('Y-m-d');
          $yearI       = (int)date('Y');
...
<head>
  ...
  <style>
    ...
    :root{ --glass-bg: rgba(17,20,28,.55); --glass-brd: rgba(255,255,255,.08); --text-dim:#9ca3af; }
    .modal-backdrop.show{ backdrop-filter: blur(6px); opacity:.5; }
    .modal-content.glass{ background:var(--glass-bg); backdrop-filter: blur(14px) saturate(120%); border:1px solid var(--glass-brd); border-radius: 1rem; box-shadow: 0 10px 30px rgba(0,0,0,.45); }
    .form-control,.form-select{ background: rgba(255,255,255,.05); border-color: rgba(255,255,255,.15); color:#e5e7eb; }
    .form-control::placeholder{ color: var(--text-dim); }
    .btn-glow:hover{ box-shadow: 0 0 .8rem rgba(99,102,241,.5); }
    .navbar { position: sticky; top: 0; z-index: 1030; }
    a, .btn-link { text-decoration: none; }
    .strike { text-decoration: line-through; opacity:.75; }
    .dot { display:inline-block; width:8px; height:8px; border-radius:50%; background:#ff4242; margin-right:6px; }
    .timer-txt { font-variant-numeric: tabular-nums; }
    .badge-ot { background:#9b87f5; } /* Purple for OT label chip */
  </style>
</head>
<body>
...
<nav class="navbar bg-info-subtle border-bottom">
  <div class="container d-flex justify-content-between align-items-center">
    ...
    <div class="d-flex gap-2">
      <button class="btn btn-success btn-glow" data-bs-toggle="modal" data-bs-target="#taskModal" onclick="openAddTask()">+ Add Task</button>
      <button class="btn btn-primary fw-bold" data-bs-toggle="modal" data-bs-target="#settingsModal">⋮</button>
    </div>
  </div>
</nav>
    ...
    <div class="table-responsive mb-4">
      <table class="table table-dark table-bordered table-striped align-middle">
        <thead class="table-light text-dark">
          <tr>
            <?php if ($C['col_num'])      : ?><th style="min-width:02%;" class="text-center">#</th><?php endif; ?>
            <?php if ($C['col_desc'])     : ?><th style="min-width:25%;">Description</th><?php endif; ?>
            <?php if ($C['col_date'])     : ?><th style="min-width:60px;">Date </th><?php endif; ?>
            <?php if ($C['col_priority']) : ?><th style="min-width:10%;" class="text-center">Priority</th><?php endif; ?>
            <?php if ($C['col_status'])   : ?><th style="min-width:10%;" class="text-center">Status</th><?php endif; ?>
            <?php if ($C['col_timer'])    : ?><th style="min-width:14%;" class="text-center">Timer</th><?php endif; ?>
            <?php if ($C['col_actions'])  : ?><th style="min-width:12%;" class="text-center">Actions</th><?php endif; ?>
            <?php if ($C['col_done'])     : ?><th style="min-width:08%;" class="text-center">Ok?</th><?php endif; ?>
          </tr>
        </thead>
        <tbody>
          <?php if (!empty($tasks)): $i=1; ?>
            <?php foreach ($tasks as $t): ?>
              ...
              ?>
              <tr>
                <?php if ($C['col_num'])      : ?><td  class="text-center"><?= $i++ ?></td><?php endif; ?>
                <?php if ($C['col_desc'])     : ?><td class="<?= $isDone ? 'strike':'' ?>"><?= htmlspecialchars($t['title']) ?></td><?php endif; ?>
                <?php if ($C['col_date'])     : ?><td><?= htmlspecialchars(fmt_range($t['date_from'], $t['date_to'])) ?></td><?php endif; ?>
                <?php if ($C['col_priority']) : ?><td class="text-center"><?= pr_badge((int)$t['priority']) ?></td><?php endif; ?>
                <?php if ($C['col_status'])   : ?><td class="text-center"><?= status_badge($t,$con) ?></td><?php endif; ?>

                <?php if ($C['col_timer']) : ?>
                  <td>
                    <?php if ($isOff): ?>
                      <?php if (!$offWorked): ?>
                        <!-- Not worked yet: show Work? button -->
                        <form class="d-inline" method="post" action="todo_list.php<?= keep_month_qs() ?>" onsubmit="return confirm('Mark this Off Day as Worked and enable O.T?');">
                      ...
                      <?php else: ?>
                        ...
                        <?php else: ?>
                          ...
                          <span class="timer-txt" data-start="<?= htmlspecialchars($startIso) ?>" id="timer-<?= $rowId ?>">00:00:00</span>
                          <form class="d-inline ms-2" method="post" action="todo_list.php<?= keep_month_qs() ?>" onsubmit="return confirm('End O.T and log to Daily Reports?');">
                            <input type="hidden" name="csrf" value="<?= htmlspecialchars($_SESSION['csrf']) ?>">
                    ...
                    <?php else: ?>
                      ...
                      <?php else: ?>
                        <span class="dot text-center" title="Running"></span>
                        <span class="timer-txt" data-start="<?= htmlspecialchars($startIso) ?>" id="timer-<?= $rowId ?>">00:00:00</span>
                        <form class="d-inline ms-2" method="post" action="todo_list.php<?= keep_month_qs() ?>" onsubmit="return confirm('End timer and log to Daily Reports?');">
                          <input type="hidden" name="csrf" value="<?= htmlspecialchars($_SESSION['csrf']) ?>">
                          <input type="hidden" name="action" value="end_timer">
                          <input type="hidden" name="id" value="<?= $rowId ?>">
                ...
                <?php if ($C['col_actions']) : ?>
                  <td class="d-flex flex-wrap gap-1 text-center justify-content-center">
                    <button type="button"
                            ...
                            data-from="<?= htmlspecialchars($t['date_from']) ?>"
                            data-to="<?= htmlspecialchars($t['date_to']) ?>"
                            data-duration="<?= htmlspecialchars($t['duration']) ?>"
                            data-priority="<?= (int)$t['priority'] ?>"
                            onclick="openEditFromButton(this)">
                      Edit
                    </button>
                    <form method="post" action="todo_list.php<?= keep_month_qs() ?>" onsubmit="return confirm('Delete this task?');">
                      <input type="hidden" name="csrf" value="<?= htmlspecialchars($_SESSION['csrf']) ?>">
        ...
        <div class="modal-body">
          <div class="row g-3">
            <div class="col-md-7">
              ...
              <input type="text" name="title" id="title" class="form-control" placeholder="Costcounter Website / Off Day / Ecommerce Website" required>
            </div>
            <div class="col-md-5">
              <label class="form-label">Duration (optional)</label>
              <input type="text" name="duration" id="duration" class="form-control" placeholder="e.g., 90 min or 1 hr 30 min">
            </div>
          ...
          <small class="text-warning d-block mt-2">Tip: <strong>Off Day</strong> লিখলে – কাজ করলে “Worked” + Timer-এ <b>O.T</b> দেখাবে, না করলে সবুজ “Off Day”.</small>
        </div>
  ...
  <div class="modal-dialog modal-md modal-dialog-centered">
    <div class="modal-content glass text-light">
      <form method="post">
        ...
        <div class="modal-body">
          <?php function chk($k){ global $C; return $C[$k] ? 'checked' : ''; } ?>
          <div class="form-check"><input class="form-check-input" type="checkbox" id="c1" name="col_num"      <?= chk('col_num') ?>><label class="form-check-label" for="c1"># (Serial)</label></div>
          <div class="form-check"><input class="form-check-input" type="checkbox" id="c2" disabled checked><label class="form-check-label" for="c2">Description (always on)</label></div>
          <div class="form-check"><input class="form-check-input" type="checkbox" id="c3" name="col_date"     <?= chk('col_date') ?>><label class="form-check-label" for="c3">Date / Range</label></div>
          <div class="form-check"><input class="form-check-input" type="checkbox" id="c4" name="col_priority" <?= chk('col_priority') ?>><label class="form-check-label" for="c4">Priority</label></div>
          <div class="form-check"><input class="form-check-input" type="checkbox" id="c5" name="col_status"   <?= chk('col_status') ?>><label class="form-check-label" for="c5">Status</label></div>
          <div class="form-check"><input class="form-check-input" type="checkbox" id="c6" name="col_timer"    <?= chk('col_timer') ?>><label class="form-check-label" for="c6">Timer</label></div>
          <div class="form-check"><input class="form-check-input" type="checkbox" id="c7" name="col_actions"  <?= chk('col_actions') ?>><label class="form-check-label" for="c7">Actions (Edit/Delete)</label></div>
          <div class="form-check"><input class="form-check-input" type="checkbox" id="c8" name="col_done"     <?= chk('col_done') ?>><label class="form-check-label" for="c8">Done/Undo</label></div>
        </div>
        <div class="modal-footer">
...
<script>
  ...
  function tickTimers(){
    ...
    nodes.forEach(el => {
      ...
      let diff = Math.max(0, Math.floor((now - st) / 1000)); // seconds
