...
if (!$report_date || $description === '') {
  $_SESSION['flash'] = ['text'=>'Date & Description required','type'=>'warning'];
...
$stmt = $con->prepare("INSERT INTO daily_reports
  (report_date,year,month,day_name,description,start_time,end_time,duration,remark,updated_at)
  VALUES (?,?,?,?,?,?,?,?,?,?)");
$stmt->bind_param("sissssssss", $report_date,$year,$month,$day_name,$description,$start_time,$end_time,$duration,$remark,$updated_at);

if ($stmt->execute()) {
  $_SESSION['flash'] = ['text'=>'Created successfully','type'=>'success'];
} else {
  $_SESSION['flash'] = ['text'=>'Create failed','type'=>'danger'];
}
