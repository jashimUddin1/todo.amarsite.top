...
function keep_month_qs($default=''){
  $y = $_GET['year'] ?? null; $m = $_GET['month'] ?? null;
...
function fmt_range($from, $to){
  ...
  if (date('mY',$f) === date('mY',$t)) return date('d', $f) . '–' . date('d', $t);
...
function status_badge($row, mysqli $con){
  if (is_off_day_title($row['title'])) {
    if (off_worked_effective($row,$con)) return '<span class="badge text-bg-warning">Worked</span>';
      ...
      <table class="table table-dark table-bordered table-striped align-middle">
        <thead class="table-light text-dark">
          <tr>
            ...
            <?php if ($C['col_desc'])     : ?><th style="min-width:165px;">Description</th><?php endif; ?>
            <?php if ($C['col_date'])     : ?><th style="min-width:60px;">Date</th><?php endif; ?>
            <?php if ($C['col_priority']) : ?><th class="text-center">Priority</th><?php endif; ?>
            <?php if ($C['col_status'])   : ?><th class="text-center">Status</th><?php endif; ?>
            <?php if ($C['col_timer'])    : ?><th class="text-center">Timer</th><?php endif; ?>
            <?php if ($C['col_actions'])  : ?><th class="text-center">Actions</th><?php endif; ?>
            <?php if ($C['col_done'])     : ?><th class="text-center">Ok?</th><?php endif; ?>
          </tr>
        </thead>
        <tbody>
