...
// todo_list.php — Todo List + Live Timer + Single-running + Off Day (Worked/OT) + Column Management
...
$con->query("
  CREATE TABLE IF NOT EXISTS todos (
    ...
    date_from_label VARCHAR(50) NULL,
    date_to_label VARCHAR(50) NULL,
    date_from DATE NULL,
    date_to DATE NULL,
...
");
$con->query("ALTER TABLE todos ADD COLUMN IF NOT EXISTS date_from_label VARCHAR(50) NULL");
$con->query("ALTER TABLE todos ADD COLUMN IF NOT EXISTS date_to_label VARCHAR(50) NULL");
...
/*
  Date label support:
  - User can type any string/number like "Day-1", "1", "Week 2".
  - If the label matches YYYY-MM-DD, we also store the real DATE for off-day logic.
*/
function parse_date_if_ymd($s)
{
  $s = trim((string) $s);
  if ($s === '')
    return null;
...
function fmt_range($from, $to, $fromLabel = null, $toLabel = null)
{
  $fromLabel = trim((string) $fromLabel);
  $toLabel = trim((string) $toLabel);
  if ($fromLabel !== '' || $toLabel !== '') {
    if ($fromLabel !== '' && $toLabel !== '')
      return htmlspecialchars($fromLabel) . '–' . htmlspecialchars($toLabel);
    if ($fromLabel !== '')
      return htmlspecialchars($fromLabel);
    return htmlspecialchars($toLabel);
  }
  if (!$from && !$to)
    return '—';
  if ($from && !$to)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($action === 'create') {
    ...
    $date_from_label = trim($_POST['date_from_label'] ?? '');
    $date_to_label = trim($_POST['date_to_label'] ?? '');
    $date_from = parse_date_if_ymd($date_from_label);
    $date_to = parse_date_if_ymd($date_to_label);
    $priority = (int) ($_POST['priority'] ?? 2);
    $duration = trim($_POST['duration'] ?? '');
    $updated_at = date("Y-m-d H:i:s");

    if ($title === '') {
      flash('Description required', 'warning');
      header("Location: todo_list.php");
      exit;
    }
    ...
    $stmt = $con->prepare("INSERT INTO todos (title,notes,date_from_label,date_to_label,date_from,date_to,duration,priority,status,updated_at,off_worked) VALUES (?,?,?,?,?,?,?,?,0,?,0)");
    $stmt->bind_param("ssssssssis", $title, $notes, $date_from_label, $date_to_label, $date_from, $date_to, $duration, $priority, $updated_at);
    $ok = $stmt->execute();
  ...
  if ($action === 'update') {
    ...
    $stmt = $con->prepare("UPDATE todos SET title=?, notes=?, date_from_label=?, date_to_label=?, date_from=?, date_to=?, duration=?, priority=?, updated_at=? WHERE id=?");
    $stmt->bind_param("sssssssssi", $title, $notes, $date_from_label, $date_to_label, $date_from, $date_to, $duration, $priority, $updated_at, $id);
    $ok = $stmt->execute();
...
$stmt = $con->prepare("
  ...
  ORDER BY status ASC,
           priority DESC,
           COALESCE(date_from, '1000-01-01') DESC,
           created_at DESC,
           id DESC
");
$stmt->execute();
...
<body>
      ...
      <div class="table-responsive mb-4">
        <table class="table table-dark table-bordered table-striped align-middle">
          ...
          <tbody>
            <?php if (!empty($tasks)):
              ...
              <?php foreach ($tasks as $t): ?>
                ...
                <tr>
                  ...
                  <?php if ($C['col_date']): ?>
                    <td><?= fmt_range($t['date_from'], $t['date_to'], $t['date_from_label'] ?? null, $t['date_to_label'] ?? null) ?></td><?php endif; ?>
                  <?php if ($C['col_priority']): ?>
                  ...
                  <?php if ($C['col_timer']): ?>
                    <td>
                      <?php if ($isOff): ?>
                        <?php if (!$offWorked): ?>
                          <form class="d-inline" method="post" action="todo_list.php"
                        ...
                        <?php else: ?>
                          <?php if (!$running): ?>
                            ...
                            <form class="d-inline start-form" method="post" action="todo_list.php">
                              <input type="hidden" name="csrf" value="<?= htmlspecialchars($_SESSION['csrf']) ?>">
                          ...
                          <?php else: ?>
                            ...
                            <form class="d-inline ms-2" method="post" action="todo_list.php"
                  ...
                  <?php if ($C['col_actions']): ?>
                    <td class="d-flex flex-wrap gap-1 text-center justify-content-center">
                      <button type="button" class="btn btn-sm btn-outline-info" data-bs-toggle="modal"
                        ...
                        data-from-label="<?= htmlspecialchars($t['date_from_label'] ?? '') ?>"
                        data-to-label="<?= htmlspecialchars($t['date_to_label'] ?? '') ?>"
            ...
            <?php else: ?>
              <tr>
                <td colspan="<?= array_sum($C) ?>" class="text-center text-secondary">No tasks found</td>
              </tr>
            <?php endif; ?>
          </tbody>
        </table>
          ...
          <div class="modal-body">
            <div class="row g-3">
              ...
              <div class="col-md-3">
                ...
                <input type="text" name="date_from_label" id="date_from_label" class="form-control"
                  placeholder="e.g., Day-1 / 1 / 2025-12-27">
                <div class="form-text text-secondary">YYYY-MM-DD দিলে Off Day auto-check কাজ করবে।</div>
              </div>
              <div class="col-md-3">
                <label class="form-label">To (optional)</label>
                <input type="text" name="date_to_label" id="date_to_label" class="form-control"
                  placeholder="e.g., Day-7 / 7 / 2025-12-31">
              </div>
              <div class="col-md-3">
  ...
  <script>
    ...
    function openAddTask() {
      ...
      document.getElementById('date_from_label').value = '';
      document.getElementById('date_to_label').value = '';
    ...
    function openEditFromButton(btn) {
      ...
      const fromLabel = btn.dataset.fromLabel || btn.dataset.from || '';
      const toLabel = btn.dataset.toLabel || btn.dataset.to || '';
      document.getElementById('date_from_label').value = fromLabel;
      document.getElementById('date_to_label').value = toLabel;
